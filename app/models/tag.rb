class Tag < ActiveRecord::Base
  ...
  def self.counts_by_day_as_rickshaw
    ...
    earliest_date = Commit.order("committed_at asc").first.committed_at

    # get the latest date
    latest_date = Commit.order("committed_at desc").first.committed_at
    ...
    Commit.all.each do |commit|
      ...
      commit.tags.each do |tag|
        ...
        series[tag.text] ||= Marshal.load(Marshal.dump(blank_data))

        #  fill in this day in that array
        tuple_for_date = series[tag.text].detect { |tuple| tuple[:x] == committed_at }
        tuple_for_date[:y] ||= 0
        tuple_for_date[:y] += 1
      end
    end
